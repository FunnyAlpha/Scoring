    ...
    for line in p_input.split('\n'):
        ...
        for rx,val in rx_dict.items():
            ...
            if re.search(rx, line):
                ...
                df_dict[val].append(temp_list)


    df = pd.DataFrame(df_dict['CREDITBUREAU'])
    #get column name
    column = df[2].unique()
    #unstack and clean dataframe
    df = df.drop(0,1).set_index([1, 2]).unstack().droplevel(0, axis=1).rename_axis(
        index=None, columns=None).reindex(column, axis=1)
    df['SK_APPLICATION'] = df_dict['SK_APPLICATION'][0][3]     
    return df

#v_dict = parse_vct(v_test)
#print(v_dict)
#df = get_df_txt('CREDITBUREAU',v_dict)
#print(df['SYSDATE'])
